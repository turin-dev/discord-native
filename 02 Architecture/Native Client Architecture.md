...
데스크톱 shell은 32px title bar, 72px guild rail, 240px channel sidebar, 48px channel header, 가변 conversation, 240px member/search panel로 구성한다. guild·user identity는 Discord CDN asset과 deterministic fallback avatar를 동일 component에서 처리한다. message list는 reverse layout으로 최신 항목을 composer 위에 고정하고, action toolbar는 hover에서만 노출하되 메뉴 anchor는 유지해 popover가 사라지지 않게 한다. UI 치수와 색상은 `discord_design_tokens.dart`를 런타임 기준으로, `design-tokens.json`을 교환 형식으로 관리한다.
