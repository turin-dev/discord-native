...
final class DiscordWorkspaceState {
  ...
  DiscordWorkspaceState payloadReceived(Map<String, Object?> payload) {
    ...
    return switch (payload['t']) {
      ...
      'CHANNEL_UNREAD_UPDATE' => _receiveChannelUnread(data),
      _ => this,
    };
  }
  ...
  DiscordWorkspaceState _receiveChannelUnread(Map<String, Object?> data) {
    final guildId = _requiredString(data['guild_id'], 'unread update.guild_id');
    final updates = Map<String, String?>.unmodifiable({
      for (final item in _readList(data['channel_unread_updates']))
        if (item is Map && item['id'] is String)
          item['id']! as String: _optionalString(item['last_message_id']),
    });
    if (updates.isEmpty) {
      return this;
    }
    return DiscordWorkspaceState.fromCollections(
      ...
      channels: [
        for (final channel in channels)
          if (channel.guildId == guildId && updates.containsKey(channel.id))
            channel.copyWith(lastMessageId: updates[channel.id])
          else
            channel,
      ],
      scheduledEvents: scheduledEvents,
